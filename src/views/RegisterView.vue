...
const { values, errors, handleSubmit, defineField } =
  useForm({
    validationSchema: yup.object({
      ...
      password: yup.string().required('Ingresar Contraseña').oneOf([yup.ref('password')], 'Las contraseñas deben coincidir'),
      repeatPassword: yup.string().required('Repetir contraseña').oneOf([yup.ref('password')], 'Las contraseñas deben coincidir')
    })
  })
const [username, usernameAttrs] = defineField('usuario')
        ...
        <v-form ref="form" v-model="valid" @submit.prevent="validateForm">
          <v-container>
            <v-col>
              ...
              <v-row cols="12" md="4">
                <v-text-field v-model="username" label="First name" hide-details required
              ...
              <v-row cols="12" md="4">
                <v-text-field v-model="repeatPassword" label="Contraseña" type="password" hide-details required
                  autocomplete="new-password"></v-text-field>
              </v-row>
              <v-row cols="12" md="4">
              ...
              <v-row cols="12" md="4">
                <RouterLink class="mt-1" to="/login" @click.native="confirmNavigation">¿Ya tienes cuenta? Ingresa aqui
                </RouterLink>
              </v-row>
